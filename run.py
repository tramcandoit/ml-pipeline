...
#mlflow.set_experiment(experiment_id="184156684729973530")
@hydra.main(config_name="config",config_path=".",version_base="1.2")
def main(cfg:DictConfig):
    base_path = hydra.utils.get_original_cwd()
    _= mlflow.run(uri=os.path.join(base_path,"data-validation"),
                   entry_point="main",
                   parameters={
    ...
    _= mlflow.run(uri=os.path.join(base_path,"feature-engineering"),
                   entry_point="main",
                   parameters={
                       'feature_store_path': cfg.data.feature_store_path,
                       'validated_train_path': cfg.data.validated_train_path
                           })
    _= mlflow.run(uri=os.path.join(base_path,'model-training'),
                   entry_point="main",
                   parameters = {
